...
namespace sales.Droid.Implementations
{
    ...
    public class LoginFacebookPageRenderer : PageRenderer
    {
        public LoginFacebookPageRenderer()
        {
            ...
            var auth = new OAuth2Authenticator(clientId: facebookAppID, scope: facebookScope, authorizeUrl: new Uri(facebookAuthURL), redirectUrl: new Uri(facebookRedirectURL));
        ...
        private async Task<TokenResponse> GetFacebookProfileAsync(string accessToken)
        {
            ...
            var token = await apiService.LoginFacebook( url, prefix, $"{controller}/LoginFacebook", facebookResponse);
            return token;
        }
    }
}
